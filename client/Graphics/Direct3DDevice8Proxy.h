class Direct3DDevice8Proxy : public IDirect3DDevice8
{
private:
	IDirect3DDevice8*	p_Dev;
public:
	Direct3DDevice8Proxy(IDirect3DDevice8* DID) : p_Dev(DID)
	{
		Core::GetCore()->GetGraphics()->Init(this);
	}
	...
	void    __stdcall Direct3DDevice8Proxy::SetCursorPosition(UINT XScreenSpace, UINT YScreenSpace, DWORD Flags)
	{
		p_Dev->SetCursorPosition(XScreenSpace, YScreenSpace, Flags);
	}
	/*STDMETHOD_(SetCursorPosition)(int x, int y, DWORD flags)
	{
	...
	{
		Core::GetCore()->GetGraphics()->OnDeviceLost();

		HRESULT hr = p_Dev->Reset(presentParam);
		if (SUCCEEDED(hr))
			Core::GetCore()->GetGraphics()->OnDeviceReset();
		return hr;
	}
	...
	{
		Core::GetCore()->GetGraphics()->OnSceneBegin();
	...
	STDMETHOD(EndScene)()
	{
		...
		Core::GetCore()->GetGraphics()->OnSceneEnd();
